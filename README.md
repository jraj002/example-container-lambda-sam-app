# example-container-lambda-sam-app
...
```bash
example-container-lambda-sam-app$ sam build
```
...
```bash
example-container-lambda-sam-app$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
example-container-lambda-sam-app$ sam local start-api
example-container-lambda-sam-app$ curl http://localhost:3000/classify_digit
```
...
```bash
example-container-lambda-sam-app$ sam logs -n InferenceFunction --stack-name example-container-lambda-sam-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name example-container-lambda-sam-app
```
